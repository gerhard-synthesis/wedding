...
function copyDetails() {
    const details = `
        Account Name: ${document.getElementById('account-name').textContent}
        Account Number: ${document.getElementById('account-number').textContent}
        Bank: ${document.getElementById('bank-name').textContent}
        Branch Code: ${document.getElementById('branch-code').textContent}
        Reference: ${document.getElementById('reference').textContent}
    `;
    
    const tempInput = document.createElement('textarea');
    tempInput.value = details.trim();
    document.body.appendChild(tempInput);
    tempInput.select();
    document.execCommand('copy');
    document.body.removeChild(tempInput);
    
    alert('EFT details copied to clipboard!');
}
// Google map
function initMap() {
    var location = {lat: -25.90253795232414, lng: 28.168786836210696};
